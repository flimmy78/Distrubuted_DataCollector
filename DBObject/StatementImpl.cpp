...
occiwrapper::StatementImpl::StatementImpl( oracle::occi::Statement* pOcciStat, 
	shared_ptr< occiwrapper::Connection >& pConn, bool bAutoCommit, const string& strTag, bool bHasCursor)
...
,m_bHasNewExtractor( false )
,m_bHasCursor( bHasCursor )
//,m_bNewSQL(true)
{
	...
	{
		m_strTag = strTag;
		this->m_objBinder = new occiwrapper::Binder( this->m_pOcciStat, pConn->GetEnvirnment(), pConn->GetOcciConnection(strTag) );
		this->m_pExtrator = new occiwrapper::Extractor( pConn->GetEnvirnment(), pConn->GetOcciConnection(strTag) );
	}
}
...
occiwrapper::StatementImpl::~StatementImpl()
{
	...
	if( m_pOcciStat )
	{
		if( !m_pConnection->TerminateStatement( m_pOcciStat, m_strTag ) )
		{
...
bool occiwrapper::StatementImpl::Execute()
{
	try
	{
		...
		if (m_pOcciResult != nullptr)
		{
			m_pOcciStat->closeResultSet(m_pOcciResult);
			m_pOcciResult = nullptr;
		}
	...
	try
	{
		...
		else
		{
			if( !m_bHasNext )
			{
				if (m_bHasCursor)
				{
					int updateCount = m_pOcciStat->execute();
					m_pOcciResult = m_pOcciStat->getCursor(1);
				}
				else
				{
...
bool occiwrapper::StatementImpl::GetTableProperty(const string& strTableName, map<UInt8, string>& mapTablePro)
{
	bool bRet = false;
	UInt32 count = 0;
	assert(m_eState != ST_DONE);
	...
	{
		///获取表字段属性
		oracle::occi::Connection* p = m_pConnection->GetOcciConnection(m_strTag);

		if (p != nullptr)
		{
			oracle::occi::MetaData custtab_metaData = p->getMetaData(strTableName, oracle::occi::MetaData::PTYPE_TABLE);
			vector <oracle::occi::MetaData> listOfColumns = custtab_metaData.getVector(oracle::occi::MetaData::ATTR_LIST_COLUMNS);
			unsigned int uiColumnNum = listOfColumns.size();

			for (unsigned int i = 0; i < uiColumnNum; i++)
			{
				oracle::occi::MetaData columnObj = listOfColumns[i];

				mapTablePro.insert(map<UInt8, string>::value_type(i,
					columnObj.getString(oracle::occi::MetaData::ATTR_NAME).c_str()));
			}
