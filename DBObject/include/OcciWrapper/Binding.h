...
{
	class BindingCursor : public AbstractBinding
	{
	public:
		/***
		*	@brief: Creates the Binding for cursor
		*/
		explicit BindingCursor(bool bBatched, occiwrapper::DataDirection_E eDataDirection = occiwrapper::PAR_OUT)
			:m_bBound(false)
			, m_bBatched(bBatched)
			, m_nBatchedNum(0)
		...
		*/
		...
		void Bind(std::size_t pos)
		{
			...
			TypeHandlerCursor::bind(pos, p_binder);
			m_bBound = true;
		}
		...
		void ReverseBind(std::size_t pos)
		{
			...
			if (this->IsOut())
			{
				...
				TypeHandlerCursor::retrieve(pos,  p_binder);
			}
		}
	...
	*/
	...
	public:
		...
		*/
		...
		void ReverseBind( std::size_t pos )
		{
			...
			{
				//p_binder->Retrieve( pos, m_objVal );
				TypeHandler< T >::retrieve( pos, m_objVal, p_binder );
			}
		}
	...
	public:
		...
		*	@brief: Creates the Binding.
		*/
		explicit Binding(const std::list<T>& val, bool bBatched, occiwrapper::DataDirection_E eDataDirection = occiwrapper::PAR_IN )
			: m_objConstVal( val )
			, m_bBound( false )
			, m_bBatched( bBatched )
			, m_nBatchedNum( 0 )
			, m_eDataDirection( m_eDataDirection )
			, m_objVal( const_cast< std::list< T > & >( val ) )
		{
		...
		std::size_t NumOfColumnsHandled() const
		{
			return TypeHandler< std::list<T> >::size();
		}
		...
		void Bind( std::size_t pos )
		{
			...
			assert( p_binder != 0 );
			if( this->IsOut() )
				throw BindException( "list can not registered as out variables" );
			TypeHandler< std::list<T> >::bind( pos, m_objConstVal, p_binder );
			m_bBound = true;
		}
	...
	private:
		const std::list< T >&				m_objConstVal;
		std::list< T >&						m_objVal;
		bool								m_bBound;
		bool								m_bBatched;
		size_t								m_nBatchedNum;
	...
	template< typename T >
	BindingCursor* useCursor(const T& t, occiwrapper::DataDirection_E dir = occiwrapper::PAR_OUT)
	{
		return new BindingCursor(false, dir);
	}
	...
	template< typename T > 
	Binding< list< T > >* use( const list< T >& t, occiwrapper::DataDirection_E dir = occiwrapper::PAR_IN );
	...
	template< typename T >
	Binding< list< T > >* batched_use( const list< T >& t )
	{
		Binding< list< T > >* p = new Binding< list< T > >( t , true );
		p->SetBatchedNum( t.size() );
		return p;
	}
}