...
namespace occiwrapper
{
	...
	*	@add by CUCmehp
	*/
	class TypeHandlerCursor
	{
	public:
		static void bind(std::size_t pos, AbstractBinder* pBinder)
		{
			assert(pBinder != 0);
			pBinder->Bind(pos);
		}

		static void retrieve(std::size_t pos, AbstractBinder* p_binder)
		{
			assert(p_binder != 0);
			p_binder->Retrieve(pos);
		}
		...
		static void fetch(std::size_t pos, AbstractExtractor* pExtractor, 
			vector< shared_ptr< OcciDataBuffer > >&  vecDataBuf, size_t nFetchedNum)
		{
			return;
		}
	...
	private:
		TypeHandlerCursor();
		~TypeHandlerCursor();
		TypeHandlerCursor(const TypeHandlerCursor&);
		TypeHandlerCursor& operator = (const TypeHandlerCursor&);
	};
	...
	*/
	...
	public:
		...
		static void extract( std::size_t pos, AbstractExtractor* pExt )
		{
			size_t nCharBufMax = nBatchedRetrieveCharBufMax;
			assert( pExt != 0 );
			// only one data to retrieve
			size_t nRowRetrieve = 1;
	...
	*/
	...
	public:
		static void bind(std::size_t pos, const vector< string >& obj, AbstractBinder* pBinder)
		{
			...
			for( size_t i = 0; i < obj.size(); ++ i )
			{
				...
				*(p_length + i ) = (UInt16)(obj[ i ].size() + 1);
			}
	...
	public:
		static void bind(std::size_t pos, const vector< struct tm >& obj, AbstractBinder* pBinder)
		{
			...
			for( size_t i = 0; i < obj.size(); i ++ )
			{
				*(p_length + i ) = (UInt16)char_buf_max;
				*( p_Ind + i ) = 0;
			}
	...
	public:
		static void bind(std::size_t pos, const list< string >& obj, AbstractBinder* pBinder)
		{
			...
			for( list< string >::const_iterator it = obj.begin(); it != obj.end(); ++ it, ++ nIndex )
			{
				...
				*(p_length + nIndex ) = (UInt16)(it->size() + 1);
			}
			pBinder->BatchedBind( pos, p, char_buf_max, p_length, p_Ind, TypeConvertor< string >::Convert() );
		}
