...
void occiwrapper::Extractor::Extract( std::size_t nPos, void* pBuf, const size_t nBufLength, 
	UInt16* pLength, Int16* pInd, enum oracle::occi::Type type )
{
	...
	try
	{
		...
		m_pOcciResultSet->setDataBuffer( nPos, m_vExtractVecDataBuf[ _index ]->m_pBuffer, 
			type, nBufLength, m_vExtractVecDataBuf[ _index ]->m_pDataLength, pInd );
	}
...
void occiwrapper::Extractor::ExtractEx()
{
	if (m_pOcciResultSet == NULL)
	{
		throw InvalidResultSetException("the ResultSet is NULL,when extract");
	...
	for (unsigned int _index = 0; _index < m_vExtractVecDataBuf.size(); _index++)
	{
		try
		{
			if (m_vExtractVecDataBuf[_index]->m_eType == oracle::occi::OCCI_SQLT_BLOB)
			{
				((oracle::occi::Blob*) m_vExtractVecDataBuf[_index]->m_pBuffer)->setEmpty(m_pOcciConn);
			}
			else if (m_vExtractVecDataBuf[_index]->m_eType == oracle::occi::OCCI_SQLT_CLOB)
			{
				((oracle::occi::Clob*) m_vExtractVecDataBuf[_index]->m_pBuffer)->setEmpty(m_pOcciConn);
			}

			m_pOcciResultSet->setDataBuffer(_index+1, m_vExtractVecDataBuf[_index]->m_pBuffer,
				m_vExtractVecDataBuf[_index]->m_eType, m_vExtractVecDataBuf[_index]->m_nDataBufMaxPerLength,
				m_vExtractVecDataBuf[_index]->m_pDataLength, m_vExtractVecDataBuf[_index]->m_pInd);
		}
		catch (oracle::occi::SQLException& exc)
		{
			string strErr = exc.what();
			throw BindException(strErr);
		}
		catch (...)
		{
			throw UnknownException("unknown exception occurs, when bind string");
