...
int main( int argc, char* argv[] )
{
	...
	string strErrOut, strDBTagOut, strTagOut;
	WSDBInterFace::IDBObject* pDBObject = NULL;
	bool bRet = WSDBInterFace::CreateDBObject(WSDBInterFace::ORCAL_TYPE, &pDBObject);
	pDBObject->OpenDB("BGD","MJL_YTH_NEW", "MJL_YTH_NEW", "192.168.242.119", 1521, strDBTagOut, strErrOut);
/*
	pDBObject->BeginBatchInsert(strDBTagOut,"occiwrapper.tbl_test2","X,Y",2,strTagOut);
	vector<WSDBInterFace::Int32> vec1;
	vector<WSDBInterFace::Int32> vec2;
	for(int i=0; i<9989; i++)
	{
		vec1.push_back(i);
		vec2.push_back(i);
	}
	pDBObject->SetInsertColData(vec1, strDBTagOut, strTagOut);
	pDBObject->SetInsertColData(vec2, strDBTagOut, strTagOut);
	pDBObject->EndBatchInsert(strDBTagOut, strTagOut, strErrOut);
*/
	WSDBInterFace::UInt32 nRowNum = 0;
	pDBObject->BeginBatchRead(strDBTagOut, "select X, Y from occiwrapper.tbl_test2", 2, strTagOut);
	vector<WSDBInterFace::Int32> vecout1;
	vector<WSDBInterFace::Int32> vecout2;
	pDBObject->SetReadColData(vecout1, strDBTagOut, strTagOut);
	pDBObject->SetReadColData(vecout2, strDBTagOut, strTagOut);
	pDBObject->ReBatchRead(strDBTagOut, strTagOut, strErrOut);
	...
	/*WSDBInterFace::Int32 nParamIn = 10000;
	string strParamOut = "";
	pDBObject->BeginProcedure(strDBTagOut, "OCCIWRAPPER.P_TEST_PROCEDURE", strTagOut,2);
	pDBObject->AddParamIn(nParamIn, strDBTagOut, strTagOut);
	pDBObject->AddParamOut(strParamOut, strDBTagOut, strTagOut);
	pDBObject->EndProcedure(strDBTagOut, strTagOut, strErrOut);*/
	return 1;
