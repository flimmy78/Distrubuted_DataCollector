...
void TestProcedure()
{
	...
	int nParIn = 1000;
	string strParOut = "";
	s << "begin OCCIWRAPPER.P_TEST_PROCEDURE( :1,:2 ); end;", use( nParIn, occiwrapper::PAR_IN ), use( strParOut, occiwrapper::PAR_OUT ), now, bRet, strErrMsg;
	assert( bRet );
...
void TestMillionDataProcess()
{
	...
	occiwrapper::Session s = sf.Create( info );
	string strErrMsg;
	s << "truncate table occiwrapper.test_batched_table", now, bRet, strErrMsg;
	assert( bRet );
	...
	for( size_t k = 0; k < nTocalBatchCount; ++ k )
	{
		...
		s << "insert into occiwrapper.test_batched_table( string_value, date_value, int_value, float_value, number_value ) values ( :1, :2, :3, :4, :5 )", batched_use( vecStr ), batched_use( vecTm ), batched_use( vecInt32 ), batched_use( vecFloat ), batched_use( vecDouble ), now, bRet, strErrMsg;
		assert( bRet );
		vecStr.clear();
		vecTm.clear();
		vecInt32.clear();
	...
	vector< float > vFloat;
	vector< double > vDouble;
	s << "select string_value, date_value, int_value, float_value, number_value from occiwrapper.test_batched_table order by int_value asc", into( vStr ), into( vDate ), into( vInt ), into( vFloat ), into( vDouble ), limit( 20000 ), now, bRet, strErrMsg;
	assert( bRet );
}
...
void TestMillionDataProcessList()
{
	...
	for( size_t k = 0; k < nTocalBatchCount; ++ k )
	{
		...
		s << "insert into occiwrapper.test_batched_table( string_value, date_value, int_value, float_value, number_value ) values ( :1, :2, :3, :4, :5 )", batched_use( listStr ), batched_use( listTm ), batched_use( listInt32 ), batched_use( listFloat ), batched_use( listDouble ), now, bRet, strErrMsg;
		assert( bRet );
		listStr.clear();
		listTm.clear();
		listInt32.clear();
